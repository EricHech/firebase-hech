...
For help managing data on the front-end, the sister package `firebase-wrapper-ui` includes a nearly exhaustive list of helper functions, hooks, context generators, and components that make working with wrapper and building a serverless architecture nearly effortless.

## Initial Setup
Pull in the `FirebaseWrapperContextProviderComponent` and use it like so:

```tsx
  <FirebaseWrapperContextProviderComponent firebaseOptions={FIREBASE_OPTIONS}>
    {yourAppHere}
  </FirebaseWrapperContextProviderComponent>
```

This will initialize the database connection.

## Authentication
We provide authentication helper functions which will work once nested in the `FirebaseWrapperContextProviderComponent`. A good place to start is to simply use the `signUp` and `signIn` functions, but check here for more:
- `firebase-wrapper/services/auth.ts`

An even faster approach would be to use the given components:
- `firebase-wrapper-ui/SignUp/index.tsx`
- `firebase-wrapper-ui/SignUp/index.tsx`

## Data Types
The first step to building a project with wrapper is to think about your data types and begin building them out. To do this, navigate from the root to `@types/firebase-wrapper/index.d.ts` and paste the following:

```ts
import * as wrapperTypes from "firebase-wrapper";

declare module "firebase-wrapper" {
  export interface FirebaseWrapperDatabase extends wrapperTypes.FirebaseWrapperDatabase {
...
This will set wrapper up to receive the proper types. Now begin building out your types, likely importing them from another file and assigning them to the relevant keys here. For example, you might have a data type for each of the following: `manager`, `user`, `profile`, `project`, and `message`.

The structure that will be saved is as follows: `data/user/{uid}/userData` where `userData` will include the information provided in your type plus a few wrapper-things (such as permissions and timestamps).

## Data Keys
A data key is simply the key for accessing a particular instance of a data type. For example, the uid of a user is the data key for the user data type. You can use Firebase Push Keys for your data keys, but when possible, FirebaseWrapper really shines when you get creative with your keys. For example, the data key to a message could be `{userId}__{projectId}__{pushKey}`. In this way, without even fetching the message data, with the key alone you would be able to fetch the user who created the message and the project that the message is under.
...
Connecting data together is how we make FirebaseWrapper relational. Any time data is created or updated (using `createData` or `updateData`) you can pass in a list of connections. You can also directly call `createConnection`. This creates a two-way connection between two data types, for example, between a user and all of the projects that user is managing.

As such, you can use wrapper functions to then get all of the connected data of a particular thing. For example, when you render a user's project list, you simply fetch all the connected projects' data for that user. Inversely, for a project, you could fetch all connected users' data.
...
Take a moment to skim these two files in order to understand the underlying functions for working with a FirebaseWrapper Database:
- `firebase-wrapper/services/client-data.ts`
- `firebase-wrapper/services/server-data.ts`

Feel free to look at `firebase-wrapper-ui/hooks`, but you will probably mainly use the following to start:
- `useOnDataKeyValue`
- `useGetDataKeyValue`
- `useOnConnectionsTypeData`

### Context Generators
One of the neatest features that FirebaseWrapper provides is a way to quickly build out your global state using context generaters. See:
- `firebase-wrapper-ui/context`
...
```tsx
import { createConnectionsTypeDataContext } from "firebase-wrapper-ui/context/createConnectionsTypeDataContext";
...
- `firebase-wrapper-ui/components`
...
```tsx
import { ItemComponentProps, useCacheHook } from "firebase-wrapper-ui";
...
export function ChatInView({
  ...
  const { user } = useFirebaseWrapperContext();
...
There are a few conventions we employ that have real consequences in using FirebaseWrapper.
...
### Null vs Undefined
Note, passing `undefined` to Firebase at any time will break. Firebase only allows `null`, which will delete the data at that location. Therefore, your types won't perfectly reflect the reality that: Firebase may return `undefined` to you when nothing is at a target database location but will break if you pass `undefined` to a database location. Similarly, you may pass `null` to a database location to delete that data, but if you fetch that location, you will get `undefined` rather than `null`. This is a Firebase quirk to be aware of. Also note, when deleting data key values (an instance of a data type, ie. a user), do not use `null`, use the given FirebaseWrapper function: `removeData`.

However, even though Firebase returns `undefined` in such cases, most FirebaseWrapper helper functions override this with `null`. We consider `undefined` values to mean that the fetching has not yet been done and `null` values to mean that the fetching was done but nothing was returned. In this way, not only do we harmonize Firebase's `undefined`/`null` descrepancy, but also we enable the developer to know if it is loading or loaded regardless of whether or not there was a valid resource at the target location just by checking the value of `undefined` vs `null`.
